...
public class JobDAO {
  ...
  private void fetchJobs() {
    ...
    try (
      ...
      while (resultSet.next()) {
        ...
        System.out.println(
          "Added " + resultSet.getString("name") + " to Static List"
        );
      }
  ...
  public void addJobToDepartment(Department department, Job job) {
    ...
    try (
      Connection c = DBConnector.getConnection();
      PreparedStatement ps = c.prepareStatement(
        "INSERT INTO job_department (job_id, department_id) " +
          "SELECT ?, ? FROM DUAL " +
          "WHERE NOT EXISTS (" +
      ...
      if (row > 0) {
        System.out.println(
          "Added " + job.getName() + " to " + department.getName()
        );
      } else {
        System.out.println(
          "Unable to add " + job.getName() + " to " + department.getName()
        );
      }
    } catch (SQLException e) {
      e.printStackTrace();
  ...
  public void getJobByDepartment(Department department) {
    ...
    try (
      ...
      while (resultSet.next()) {
        department
          .getJobs()
          .add(new Job(resultSet.getInt("id"), resultSet.getString("name")));
