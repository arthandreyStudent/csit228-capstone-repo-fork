    ...
    try (
      ...
      try (ResultSet rs = stmt.executeQuery()) {
        if (rs.next()) {
          ...
          TicketStatus status =
            statusString != null ? TicketStatus.valueOf(statusString) : null;

          Integer assignedTo = rs.getObject("assigned_to", Integer.class);
          Integer departmentId = rs.getObject("department_id", Integer.class);

          LocalDateTime deadline = rs.getObject(
            "deadline",
            LocalDateTime.class
          );
          LocalDateTime dateCreated = rs.getObject(
            "date_created",
            LocalDateTime.class
          );
          LocalDateTime lastUpdated = rs.getObject(
            "last_updated",
            LocalDateTime.class
          );
