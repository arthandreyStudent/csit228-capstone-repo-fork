...
public class TicketDAO {
    ...
    try (Connection connection = DBConnector.getConnection();
      ...
      try (ResultSet rs = stmt.executeQuery()) {
        if (rs.next()) {
          ...
          TicketPriority priority = priorityString != null ? TicketPriority.valueOf(priorityString) : null;
          
          String statusString = rs.getString("status");
          TicketStatus status = statusString != null ? TicketStatus.valueOf(statusString) : null;
          ...
          return new Ticket(rs.getInt("id"), rs.getString("title"), rs.getString("description"), priority, deadline,
                            status, rs.getInt("created_by"), assignedTo, dateCreated, lastUpdated, departmentId);
        }
      }
    } catch (SQLException e) {
      e.printStackTrace();
  ...
  private void refreshTicketViews() {
    ...
    try (Connection connection = DBConnector.getConnection(); PreparedStatement stmt = connection.prepareStatement(sql);
         ResultSet rs = stmt.executeQuery()) {
      while (rs.next()) {
        tickets.add(
          new TicketView(rs.getInt("id"), rs.getString("title"), rs.getString("description"), rs.getString("priority"),
                         rs.getString("status"), rs.getString("department_name"), rs.getString("created_by"),
                         rs.getString("assigned_to_name"), rs.getObject("last_updated", LocalDateTime.class),
                         rs.getObject("date_created", LocalDateTime.class),
                         rs.getObject("deadline", LocalDateTime.class)));
  ...
  public static void main(String[] args) {
    ...
    td.getTicketViews();
    td.createTicket(new Ticket("JEK GWAPO", "WAL TEST 2 A LANG TEST LANG", TicketPriority.HIGH, LocalDateTime.now(),
                               TicketStatus.OPEN, 15, 3, LocalDateTime.now(), LocalDateTime.now(), 1));
