...
public class DashboardExecutiveController extends StaffDashboardController {
    ...
    private void renderDepartmentTabs() {
        ...
        allButton.setOnAction(event -> { selectedDepartmentName = null; renderDepartmentTabs(); loadPendingAssignmentQueue(); });
        departmentTabsBox.getChildren().add(allButton);

        Button volunteerButton = createTabButton(VOLUNTEER_TAB_NAME, VOLUNTEER_TAB_NAME.equalsIgnoreCase(selectedDepartmentName));
        volunteerButton.setOnAction(event -> { selectedDepartmentName = VOLUNTEER_TAB_NAME; renderDepartmentTabs(); loadPendingAssignmentQueue(); });
        departmentTabsBox.getChildren().add(volunteerButton);

        for (Department department : departments) {
            String name = department.getName();
            if (name != null && name.equalsIgnoreCase(VOLUNTEER_TAB_NAME)) continue;

            Button btn = createTabButton(name, name != null && name.equalsIgnoreCase(selectedDepartmentName));
            btn.setOnAction(event -> { selectedDepartmentName = name; renderDepartmentTabs(); loadPendingAssignmentQueue(); });
    ...
    private void updateSummaryCardsAndResolutionRate() {
        ...
        for (TicketView ticket : tickets) {
            if (isUnassigned(ticket))                                    unassigned++;
            if (isStatus(ticket, TicketStatus.IN_PROGRESS.name()))       inProgress++;
            if (isResolved(ticket))                                       resolved++;
    ...
    private void loadPendingAssignmentQueue() {
        ...
        for (TicketView ticket : getSortedTicketsByDeadline()) {
            if (!isAssignableTicket(ticket)) continue;
            if (!matchesSelectedDepartment(ticket)) continue;
    ...
    private boolean matchesSelectedDepartment(TicketView ticket) {
        if (selectedDepartmentName == null || selectedDepartmentName.trim().isEmpty()) return true;
    ...
    public void handleCreateTicket() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource(
                    "/com/csit228/capstone/view/CreateTicketModalExecView.fxml"));
