...
public class DashboardExecutiveController extends StaffDashboardController {
  ...
  private void renderDepartmentTabs() {
    ...
    Button allButton = createTabButton(
      "All Depts",
    ...
    Button volunteerButton = createTabButton(
      VOLUNTEER_TAB_NAME,
    ...
    for (Department department : departments) {
      ...
      Button btn = createTabButton(
        name,
        name != null && name.equalsIgnoreCase(selectedDepartmentName)
      );
  ...
  private void loadPendingAssignmentQueue() {
    ...
    for (TicketView ticket : getSortedTicketsByDeadline()) {
      ...
      ListRowItem row = ListRowItem.forExecutiveAssignment(
        ticket,
        assignableUsers
      );
  ...
  private boolean matchesSelectedDepartment(TicketView ticket) {
    if (
      selectedDepartmentName == null || selectedDepartmentName.trim().isEmpty()
    ) return true;
    if (
      VOLUNTEER_TAB_NAME.equalsIgnoreCase(selectedDepartmentName)
    ) return isVolunteerTicket(ticket);
    return (
      ticket.getDepartmentName() != null &&
