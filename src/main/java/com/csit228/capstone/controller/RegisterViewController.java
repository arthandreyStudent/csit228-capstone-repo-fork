...
public class RegisterViewController implements Initializable {
    ...
    public void createAccount() throws IOException {
        ...
        if(firstname.isBlank() || lastname.isBlank() || username.isBlank() || password.isBlank() || confirmPassword.isBlank() || job.isBlank()){
            showError("Please fill in all fields!");
        } else if (confirmPassword.equals(password)){
            closeError();
            try{
                userDAO.createUser(UserFactory.createUser(
                        Role.MEMBER, 1, firstname, lastname, username, Hash.hashWithSHA256(confirmPassword), department_ID
                ));
